  ...
  <div class="section-wrapper">
        ...
        <div class="container">
         ...
         <div class="row pl-5 pr-5 my-3">
          <div class="col-12 table-responsive">
           ...
           foreach ($total_test_count as $key1 => $value1) {

            $total_p_c+=$value1['paid_amount'];
            $total_d_c+=$value1['total_commission']-$value1['paid_amount'];

          } ?>

          <table class="table-bordered table-sm test_table_report">
            <tr>
            ...
            <tbody>
             ...
             foreach ($col_from_doc as $key => $value) {
               ...
               if($value['quack_doc_id']==$doc_in['doctor_id']){
                 ...
                 foreach ($total_test_count as $key1 => $value2) {
                   if($value['order_id']==$value2['order_id'])
                   {
                    $total_count=$value2['test_count'];
         ...
         // $total_count+=$value['total_test'];
                $total_p+=$value['price'];
                $total_v+=$value['vat']/$total_count;
                $total_d+=$value['discount']/$total_count;
                $total_n+=$value['price']-$value['discount']/$total_count+$value['vat']/$total_count;
                ...
                <tr>
                  ...
                  <td><?=number_format($value['vat']/$total_count,2,'.', '')?></td>
                  <td><?=number_format($value['discount']/$total_count,2,'.', '')?></td>
                  <td><?=number_format($value['price']-$value['sub_amount']+$value['vat'],2,'.', '')?></td>

                  <td><?=number_format(($value['gross_amount']/$value['price'])*100,2,'.', '').'%'?></td>
    ...
    <tbody>
     ...
     foreach ($col_from_doc as $key => $value) {
       ...
       foreach ($total_test_count as $key1 => $value1) {
         if($value['order_id']==$value1['order_id'])
         {
          $total_count=$value1['test_count'];
