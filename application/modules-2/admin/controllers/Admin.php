...
class Admin extends MX_Controller {   
	...
	public function birth_certificate_list_dt($value='')
	{
		...
		$output = array(  
			...
			"recordsFiltered"     =>     $this->admin_model->get_filtered_data_two_table_join('birth_death_certificate b','b.status=1 and is_birth_death=1',$select_column,$order_column,$search_column,'room r','r.id=b.cabin_no','b.id'
		),  
			"data"                    =>     $data  
		);  
		echo json_encode($output);
	...
	public function death_certificate_list_dt($value='')
	{
		...
		$output = array(  
			...
			"recordsFiltered"     =>     $this->admin_model->get_filtered_data_two_table_join('birth_death_certificate b','b.status=1 and is_birth_death=2',$select_column,$order_column,$search_column,'room r','r.id=b.cabin_no','b.id'
		),  
			"data"                    =>     $data  
		);  
		echo json_encode($output);
...
// *********************  Manage Birth/Death Certificate Ends **************
	...
	public function uhid_list_dt($value='')
	{
		...
		$output = array(  
			...
			"recordsFiltered"     =>     $this->admin_model->get_filtered_data_two_table_join('uhid u','u.status=1',$select_column,$order_column,$search_column,'blood_group b','b.id=u.blood_group','u.id'
		),  
			"data"                    =>     $data  
		);  
		echo json_encode($output);
	...
	{
		$data = array('sub_test_title' =>$this->input->post('additional_test_name'),
			'price' =>$this->input->post('price'),
	 		...
	 		if(in_array($data['group_id'][0]['mtest_id'], explode(',', $value['group_id'])))
	 		{
	 			...
	 			foreach ($this->cart->contents() as $key => $value1) {
	 				...
	 				if($value1['options']['type']==2)
	 				{
	 					...
	 					$data3[$key]['qty']=1;
	 					$data3[$key]['price']=$value1['price'];
	 					$data3[$key]['options']=array("quk_ref_com" =>0,
	 						"discount"=>$_POST["total_discount"],
	 ...
	 public function opd_all_billing_info_dt($uhid='')
	 {
	 	...
	 	else
	 	{
	 		...
	 		$output = array(  
	 			...
	 			"recordsFiltered"     =>     $this->admin_model->get_filtered_data_two_table_join('opd_patient_test_order_info p','p.status=1 AND o.status=1 AND p.uhid="'.$uhid.'"',$select_column,$order_column,$search_column,'opd_patient_info o','o.id=p.patient_id','p_id'
	 		),  
	 			"data"                    =>     $data  
	 		);  
	 	}
...
// }
    ...
    // redirect('admin/opd_doc_com_pview',$data);
	 ...
	 public function outdoor_due_collection($value='')
	 {
	 	...
	 	$data['all_opd_patient_id']=$this->admin_model->select_with_where2('*','status=1 AND payment_status="unPaid"','opd_patient_test_order_info');
	 ...
	 public function get_patient_bill_info_by_patient_id($value='')
	 {
	 	...
	 	$data["patient_order_info"]=$this->admin_model->select_with_where2('*','id="'.$order_id.'" AND status=1','opd_patient_test_order_info');

	 	$data['patient_id']=$data["patient_order_info"][0]['patient_id'];


	 	$data['patient_info']=$this->admin_model->select_with_where2('*','id="'.$data["patient_order_info"][0]['patient_id'].'"','opd_patient_info');
...
//    }	
	...
	// "<pre>";print_r($data["ipd_collection_info"]);die();
	 ...
	 public function opd_col_from_doc_all($value='')
	 {
	 	$data['active']='opd_col_from_doc_all';
	 	$data['page_title']='Opd Collection Report';
	 	$data['admin_type']=$this->session->userdata['logged_in']['role'];

	 	$data['doc_list']=$this->admin_model->select_with_where2('*','status=1','doctor');

	 	$this->load->view('opd/opd_col_from_doc_all',$data);

	 }

	 public function opd_col_from_doc_all_report($value='')
	 {
	 	$start_date=$this->input->get('start_date'); 
	 	$end_date=$this->input->get('end_date');
	 	$doc_id=$this->input->get('doc_name');

	 	if($doc_id=="all")
	 	{
	 		$data["col_from_doc"]=$this->admin_model->select_join_two_table2_sum_three('o.*, d.*, o.created_at','opd_patient_test_order_info o','due_collection d','o.test_order_id=d.order_id','date(d.created_at) between "'.$start_date.'" AND "'.$end_date.'" and d.due_type=1','d.vat','d.discount','d.paid_due','o.id');
	 	...
	 	else if($doc_id=="all_mbbs")
	 	{
	 		$data["col_from_doc"]=$this->admin_model->select_join_two_table2_sum_three('o.*, d.*, o.created_at','opd_patient_test_order_info o','due_collection d','o.test_order_id=d.order_id','date(d.created_at) between "'.$start_date.'" AND "'.$end_date.'" and d.due_type=1','d.vat','d.discount','d.paid_due','o.id');

	 		$data['doc_info']=$this->admin_model->select_with_where2('*','status=1 and doctor_type=1','doctor');

	 		$data['flag']="all";
	 	}

	 	else if ($doc_id=="all_quack"){

	 		$data["col_from_doc"]=$this->admin_model->select_join_two_table2_sum_three('o.*, d.*, o.created_at','opd_patient_test_order_info o','due_collection d','o.test_order_id=d.order_id','date(d.created_at) between "'.$start_date.'" AND "'.$end_date.'" and d.due_type=1','d.vat','d.discount','d.paid_due','o.id');

	 		$data['doc_info']=$this->admin_model->select_with_where2('*','status=1 and doctor_type=2','doctor');
	 	...
	 	$this->load->view('opd/opd_col_from_doc_all_report',$data);
	 ...
	 public function opd_col_from_doc_with_com_next($start_date,$end_date,$doc_id)
	 {
	 	...
	 	if($doc_id=="all")
	 	{
	 		$data["col_from_doc"]=$this->admin_model->select_join_five_table2_sum_six_count_group_by_right('o.quack_doc_id,d.paid_amount,count(o.id) as total_test,o.id as order_id,,o.test_order_id,dt.test_title,dt.test_id,dts.sub_test_title,dts.price,o.patient_id,o.total_amount,o.vat,o.total_discount as discount,o.created_at as c_date,o.quack_doc_id,o.ref_doc_id,dc.amount,dc.gross_amount,d.total_commission,d.paid_amount,d.total_gross_com,d.doc_id','doc_commission d','doc_commission_details dc','opd_patient_test_order_info o','diagnostic_test_subgroup dts','diagnostic_test_group dt','d.id=dc.com_id','d.order_id=o.id','dc.service_id=dts.id','dts.mtest_id=dt.test_id','date(d.created_at) between "'.$start_date.'" AND "'.$end_date.'"','o.vat','o.total_discount','dts.price','dc.amount','dc.sub_amount','dc.gross_amount','o.id');

	 		$data['doc_info']=$this->admin_model->select_with_where2('*','status=1','doctor');

	 		// $data['doc_info']=$this->admin_model->select_join_where_group_by_right('*','doc_commission dc','doctor d','d.doctor_id=dc.doc_id','d.status=1 and dc.status=1 and d.doctor_type=1 and date(dc.created_at) between "'.$start_date.'" AND "'.$end_date.'"','d.doctor_id');

	 		// "<pre>";print_r($data['col_from_doc']);die();

	 		$data['flag']="all";
	 		$data['flag_col']="ref_doc_id";
	 	}

	 	else if($doc_id=="all_mbbs")
	 	{
	 		$data["col_from_doc"]=$this->admin_model->select_join_five_table2_sum_six_count_group_by_right('o.quack_doc_id,d.paid_amount,count(o.id) as total_test,o.id as order_id,,o.test_order_id,dt.test_title,dt.test_id,dts.sub_test_title,dts.price,o.patient_id,o.total_amount,o.vat,o.total_discount as discount,o.created_at as c_date,o.quack_doc_id,o.ref_doc_id,dc.amount,dc.gross_amount,d.doc_id,d.total_commission,d.paid_amount,d.total_gross_com','doc_commission d','doc_commission_details dc','opd_patient_test_order_info o','diagnostic_test_subgroup dts','diagnostic_test_group dt','d.id=dc.com_id','d.order_id=o.id','dc.service_id=dts.id','dts.mtest_id=dt.test_id','date(d.created_at) between "'.$start_date.'" AND "'.$end_date.'"','o.vat','o.total_discount','dts.price','dc.amount','dc.sub_amount','dc.gross_amount','o.id');
	 		...
	 		// $data['doc_info']=$this->admin_model->select_join_where_group_by_right('*','doc_commission dc','doctor d','d.doctor_id=dc.doc_id','d.status=1 and dc.status=1 and d.doctor_type=2 and date(dc.created_at) between "'.$start_date.'" AND "'.$end_date.'"','d.doctor_id');
	 	...
	 	{
	 		$data["col_from_doc"]=$this->admin_model->select_join_five_table2_sum_six_count_group_by('d.paid_amount,count(o.id) as total_test,o.id as order_id,,o.test_order_id,dt.test_title,dt.test_id,dts.sub_test_title,dts.price,o.patient_id,o.total_amount,o.vat,o.total_discount as discount,o.created_at as c_date,o.ref_doc_id,o.quack_doc_id,d.total_commission,d.paid_amount,d.total_gross_com','doc_commission d','doc_commission_details dc','opd_patient_test_order_info o','diagnostic_test_subgroup dts','diagnostic_test_group dt','d.id=dc.com_id','d.order_id=o.id','dc.service_id=dts.id','dts.mtest_id=dt.test_id','d.doc_id="'.$doc_id.'" AND date(d.created_at) between "'.$start_date.'" AND "'.$end_date.'"','o.vat','o.total_discount','dts.price','dc.amount','dc.gross_amount','dc.sub_amount','o.id');
	 ...
	 public function opd_col_from_doc_with_com_details($value='')
	 {
	 	...
	 	$data['doc_list']=$data['doc_list']=$this->admin_model->select_with_where2('*','status=1','doctor');
	 ...
	 public function ipd_adv_pay_pdf($p_id='')
	 {
	 	...
	 	#$data['patient_info']=$this->admin_model->select_join_where('*','ipd_patient_info i','room r','i.cabin_no = r.id','i.status=1 AND i.id="'.$p_id.'"',);
	 ...
	 public function ipd_adv_pay_post($patient_id='')
	 {
	  ...
	  // update ipd final bill 
	 	...
	 	$data = array('advance_payment' => $ipd_final_bill[0]['advance_payment']+$_POST['adv_pay'],
	 ...
	 public function discharge_certificate($pres_id='',$patient_id='')
	 {
	 	...
	 	$data['dr_info']=$this->admin_model->select_with_where2('*','status=1 AND doctor_id="'.$data['patient_info'][0]['doc_id'].'"','doctor');

	 	$data['uhid_info']=$this->admin_model->select_with_where2('*','status=1 AND id="'.$data['patient_info'][0]['uhid'].'"','uhid');
	 ...
	 public function ipd_reg_form($ipd_patient_id='')
	 {
	 	...
	 	$data['ipd_patient_info']=$this->admin_model->select_join_four_table2_left('*,i.created_at,i.*','ipd_patient_info i','room r','blood_group b','uhid u','i.cabin_no=r.id','b.id=i.blood_group','i.uhid=u.id','i.status=1 and i.id="'.$ipd_patient_id.'"');
	 ...
	 public function ipd_registration($ipd_patient_id='')
	 {
		...
		//check the cabin name is empty or not
	 		...
	 		else
	 		{
	 			...
	 			// if($this->admin_model->check_row('*','uhid="'.$this->input->post('uhid').'" and uhid!=0 and type!=3','ipd_patient_info'))
	 			// {
	 			// 	$data['message']="patient yet not released, new admission not possible";
	 			// 	$this->load->view('ipd/ipd_registration',$data);
	 			// }

	 			// else if($this->admin_model->check_row('*','uhid="'.$this->input->post('uhid').'" and uhid!=0','ipd_patient_info'))
	 			// {
	 			// 	$p_info=$this->admin_model->select_with_where2('*','status=1 and uhid="'.$this->input->post('uhid').'"','ipd_patient_info');

	 			// 	$id=$p_info[0]['id'];

	 			// 	$val1 = array('patient_name' => $this->input->post('patient_name'),
	 			// 		'reg_id' => $this->input->post('reg_no'),
	 			// 		'uhid' => $this->input->post('uhid'),
	 			// 		'mobile_no' => $this->input->post('phone_no'),
	 			// 		'description' => $this->input->post('description'),
	 			...
	 			$d_data['current_due']=$admission_fee-$admission_fee_paid;
	 ...
	 public function ipd_all_patient_list_dt($value='')
	 {
	 	...
	 	$output = array(  
	 		...
	 		"recordsFiltered"     =>     $this->admin_model->get_filtered_data_two_table_join('ipd_patient_info i',$condition,$select_column,$order_column,$search_column,'room r','r.id=i.cabin_no','i.id'
	 	),  
	 		"data"                    =>     $data  
	 	);  
	 	echo json_encode($output);
	 ...
	 public function get_ipd_patient_billing_info_pdf1()
	 {
	 	...
	 	$data['patient_info']=$this->admin_model->select_join_where_left('*,i.id,i.*','ipd_patient_info i','uhid u','i.uhid=u.id','i.status=1 AND i.id="'.$patient_id.'"');
	 ...
	 public function ipd_update_payment($value='')
	 {
	 	...
	 	if(date("Y-m-d", strtotime($data['ipd_final_bill_info'][0]['released_date'] )) != date('Y-m-d'))
	 	{
	 		$d_data['is_due_collection']=1;
	 	}
	 	...
	 	$d_data['vat']=$this->input->post('vat');
	 	$d_data['discount']=$this->input->post('discount');
	 	$d_data['current_due']=$this->input->post('due')-$this->input->post('update_payment')-$this->input->post('discount');
	 	$d_data['paid_due']=$this->input->post('update_payment');
	 ...
	 public function indoor_due_collection($value='')
	 {
	 	...
	 	$data['all_ipd_patient_id']=$this->admin_model->select_join_where('*','ipd_patient_info i','ipd_final_bill b','i.id=b.p_id','i.type =3  and i.status=1 AND b.payment_status="unpaid"');
	 ...
	 public function date_wise_indoor_collection($value='')
	 {
	 	...
	 	$data["patient_test_order_info"]=$this->admin_model->get_four_charge_sum_where_group_by_join_two('*,due_collection.created_at,ipd_final_bill.total_amount','paid_due','discount','due_collection.admission_fee_paid','due_collection.advance_payment','due_collection.patient_id','ipd_patient_info','due_collection','ipd_patient_info.id=due_collection.patient_id','ipd_final_bill','ipd_final_bill.p_id=ipd_patient_info.id',' due_collection.status=1 and date(due_collection.created_at)="'.date('Y-m-d').'" AND due_collection.due_type=2');
	...
	//*************** Pharmacy Module Starts *******************/
	 ...
	 {
	 	$data = array('rack_title' =>$_POST['u_name'],
	 ...
	 public function update_cust_payment($sell_id='',$cust_id='',$flag='')
	 {
	 	...
	 	$net_total=$val[0]['net_total']-$this->input->post('discount');


	 	$val1 = array(
	 		'debit' =>$debit,
	 		'discount' =>$val[0]['discount']+$this->input->post('discount'),
	 		'net_total' =>$net_total
	 	);
	 	...
	 	$d_data['current_due']=$this->input->post('grand_due')-$this->input->post('update_payment_cust');
	 ...
	 public function user_wise_collection_pharmacy($value='')
	 {
	 	$data['active']='user_wise_collection_pharmacy';
	 	$data['page_title']='Pharmacy Collection By User';

	 	$data["user_info"]=$this->admin_model->select_with_where2('*','status=1 and role not in (0)','login');

	 	$this->load->view('pharmacy/user_wise_collection_pharmacy',$data);
	 }

	 public function user_wise_collection_pharmacy_report($value='')
	 {
	 	$data['active']='pharmacy';
	 	$data['admin_type']=$this->session->userdata['logged_in']['role'];
	 	...
	 	if($operator_id == "all")
	 	{
	 		...
	 		$data["phar_order_info"]=$this->admin_model->select_join_where('*,d.created_at, d.operator_name,d.operator_id','pharmacy_due_collection d','customer c','d.supp_cust_id=c.id','date(d.created_at) between "'.$start_date.'" AND "'.$end_date.'" AND d.due_type=2 and d.status=1');
	 	}

	 	else {

	 		$data['flag']="individual";
	 		$data["phar_order_info"]=$this->admin_model->select_join_where('*,d.created_at, d.operator_name,d.operator_id','pharmacy_due_collection d','customer c','d.supp_cust_id=c.id','date(d.created_at) between "'.$start_date.'" AND "'.$end_date.'" AND d.due_type=2 and d.operator_id="'.$operator_id.'" and d.status=1');

	 	}

	 	$this->load->view('pharmacy/user_wise_collection_pharmacy_report',$data);
	 }
	...
	// other head expense
	 ...
	 public function date_wise_balance_sheet($value='')
	 {
		...
		// outdoor
	 	...
	 	$data['outdoor_total_paid']=$this->admin_model->get_charge_sum_where('paid_due','due_collection','date(created_at)="'.date('Y-m-d').'" AND status=1 and is_due_collection=0 and due_type=1');

	 	$data['outdoor_total_paid_today']=$this->admin_model->get_charge_sum_where_join('*','d.paid_due','due_collection d','due_type=1 AND date(o.created_at)="'.date('Y-m-d').'" AND date(d.created_at)="'.date('Y-m-d').'" AND d.status=1 AND o.status=1','opd_patient_test_order_info o','o.test_order_id=d.order_id');

	 	$data['outdoor_net_income']=$this->admin_model->get_charge_sum_where('paid_due','due_collection','due_type=1 AND date(created_at)="'.date('Y-m-d').'" AND status=1');

	 	$data['outdoor_vat_income']=$this->admin_model->get_charge_sum_where('vat','due_collection','due_type=1 AND date(created_at)="'.date('Y-m-d').'" AND status=1');

	 	$data['outdoor_discount_expense_today']=$this->admin_model->get_charge_sum_where_join('*','discount','due_collection d','due_type=1 AND date(o.created_at)="'.date('Y-m-d').'" AND date(d.created_at)="'.date('Y-m-d').'" AND d.status=1 AND o.status=1','opd_patient_test_order_info o','o.test_order_id=d.order_id');
		...
		// "<pre>";print_r($data['outdoor_discount_expense']);die();
	 	...
	 	$data['outdoor_due_collection']=$this->admin_model->get_charge_sum_where('paid_due','due_collection','due_type=1 and is_due_collection=1 AND date(created_at)="'.date('Y-m-d').'" AND status=1');

	// Indoor

	 	$data['indoor_total_amount']=$this->admin_model->get_charge_sum_where('total_amount','ipd_final_bill','date(released_date)="'.date('Y-m-d').'"');

	 	$data['indoor_total_adm_fee']=$this->admin_model->get_charge_sum_where('admission_fee','ipd_final_bill','date(released_date)="'.date('Y-m-d').'"');

	 	$data['indoor_total_adm_fee_today_created']=$this->admin_model->get_charge_sum_where('admission_fee','ipd_final_bill','date(created_at)="'.date('Y-m-d').'"');


	 	$data['indoor_total_due']=$this->admin_model->indoor_bl_current_due(date('Y-m-d'));

	 	$data['indoor_total_paid']=$this->admin_model->get_charge_sum_where_join_no_selector('paid_due','due_collection d','d.status=1 and d.is_due_collection=0 and d.due_type=2 AND date(d.created_at)="'.date('Y-m-d').'" and date(i.created_at)="'.date('Y-m-d').'"','ipd_final_bill i','d.patient_id=i.p_id');
	 	...
	 	$data['indoor_due_collection']=$this->admin_model->get_charge_sum_where('paid_due','due_collection','status=1 and due_type=2 and is_due_collection=1 and date(created_at)="'.date('Y-m-d').'"');
	 	...
	 	$data['indoor_vat_income_today']=$this->admin_model->get_charge_sum_where_join_no_selector('d.vat','due_collection d','d.status=1 and d.due_type=2 AND date(d.created_at)="'.date('Y-m-d').'" AND date(i.created_at)="'.date('Y-m-d').'"','ipd_final_bill i','d.patient_id=i.p_id');
	 	...
	 	$data['indoor_adm_fee_income_today']=$this->admin_model->get_charge_sum_where_join_no_selector('d.admission_fee_paid','due_collection d','d.status=1 and d.due_type=2 AND date(d.created_at)="'.date('Y-m-d').'" AND date(i.created_at)="'.date('Y-m-d').'"','ipd_final_bill i','d.patient_id=i.p_id');


	 	$data['indoor_due_admission_fee']=$this->admin_model->get_charge_sum_where('admission_fee_paid','due_collection','status=1 and due_type=2 and is_due_collection=1 and date(created_at)="'.date('Y-m-d').'"');

	 		// "<pre>";print_r($data['indoor_total_paid']);die();

	 	$data['indoor_discount_expense_today_only']=$this->admin_model->get_charge_sum_where_join_no_selector('d.discount','due_collection d','d.status=1 and d.due_type=2 AND date(d.created_at)="'.date('Y-m-d').'" AND date(i.created_at)="'.date('Y-m-d').'"','ipd_final_bill i','d.patient_id=i.p_id');
		...
		// Indoor Diagnostic Service
	 	...
	 	$data['indoor_diag_vat_income_today']=$this->admin_model->get_charge_sum_where_join_no_selector('d.vat','due_collection d','d.status=1 and d.due_type=3 AND date(d.created_at)="'.date('Y-m-d').'" AND date(o.created_at)="'.date('Y-m-d').'"','outdoor_service_order_info o','d.order_id=o.reg_id');
	 	...
	 	$data['indoor_diag_discount_income_today']=$this->admin_model->get_charge_sum_where_join_no_selector('d.discount','due_collection d','d.status=1 and d.due_type=3 AND date(d.created_at)="'.date('Y-m-d').'" AND date(o.created_at)="'.date('Y-m-d').'"','outdoor_service_order_info o','d.order_id=o.reg_id');
	...
	// pharmacy
	 ...
	 public function date_wise_balance_sheet_report_next($start_date,$end_date,$cat)
	 {
		...
		//Outdoor
	 	...
	 	$data['outdoor_total_paid_today']=$this->admin_model->get_charge_sum_where_join('*','d.paid_due','due_collection d','due_type=1 AND date(o.created_at) between "'.$start_date.'" and "'.$end_date.'" AND date(d.created_at) between "'.$start_date.'" and "'.$end_date.'" AND d.status=1 AND o.status=1','opd_patient_test_order_info o','o.test_order_id=d.order_id');
	 	...
	 	$data['outdoor_due_collection']=$this->admin_model->get_charge_sum_where('paid_due','due_collection','due_type=1 and is_due_collection=1 AND date(created_at) between "'.$start_date.'" and "'.$end_date.'" AND status=1');
		...
		// new Indoor
	 	...
	 	$data['indoor_total_amount']=$this->admin_model->get_charge_sum_where('total_amount','ipd_final_bill','date(released_date) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['indoor_total_adm_fee']=$this->admin_model->get_charge_sum_where('admission_fee','ipd_final_bill','date(released_date) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['indoor_total_adm_fee_today_created']=$this->admin_model->get_charge_sum_where('admission_fee','ipd_final_bill','date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['indoor_total_due']=$this->admin_model->indoor_bl_current_due_datewise($start_date,$end_date);


	 	$data['indoor_total_paid']=$this->admin_model->get_charge_sum_where_join_no_selector('paid_due','due_collection d','d.status=1 and d.due_type=2 AND date(d.created_at) between "'.$start_date.'" and "'.$end_date.'" and date(i.created_at) between "'.$start_date.'" and "'.$end_date.'"','ipd_final_bill i','d.patient_id=i.p_id');

	 	// today current collection
	 	$data['indoor_net_income']=$this->admin_model->get_charge_sum_where('paid_due','due_collection','status=1 and due_type=2 and date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 	// today due collection
	 	$data['indoor_due_collection']=$this->admin_model->get_charge_sum_where('paid_due','due_collection','status=1 and due_type=2 and is_due_collection=1 and date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['indoor_vat_income']=$this->admin_model->get_charge_sum_where('vat','due_collection','status=1 and due_type=2 AND date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['indoor_vat_income_today']=$this->admin_model->get_charge_sum_where_join_no_selector('d.vat','due_collection d','d.status=1 and d.due_type=2 AND date(d.created_at) between "'.$start_date.'" and "'.$end_date.'" AND date(i.created_at) between "'.$start_date.'" and "'.$end_date.'"','ipd_final_bill i','d.patient_id=i.p_id');

	 	$data['indoor_adm_fee_income']=$this->admin_model->get_charge_sum_where('admission_fee_paid','due_collection','status=1 and due_type=2 AND date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['indoor_adm_fee_income_today']=$this->admin_model->get_charge_sum_where_join_no_selector('d.admission_fee_paid','due_collection d','d.status=1 and d.due_type=2 AND date(d.created_at) between "'.$start_date.'" and "'.$end_date.'" AND date(i.created_at) between "'.$start_date.'" and "'.$end_date.'"','ipd_final_bill i','d.patient_id=i.p_id');

	 	$data['indoor_due_admission_fee']=$this->admin_model->get_charge_sum_where('admission_fee_paid','due_collection','status=1 and due_type=2 and is_due_collection=1 and date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 		// "<pre>";print_r($data['indoor_total_paid']);die();

	 	$data['indoor_discount_expense_today_only']=$this->admin_model->get_charge_sum_where_join_no_selector('d.discount','due_collection d','d.status=1 and d.due_type=2 AND date(d.created_at) between "'.$start_date.'" and "'.$end_date.'" AND date(i.created_at) between "'.$start_date.'" and "'.$end_date.'"','ipd_final_bill i','d.patient_id=i.p_id');

	 	$data['indoor_discount_expense']=$this->admin_model->get_charge_sum_where('discount','due_collection','status=1 and due_type=2 AND date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['indoor_advance_payment']=$this->admin_model->get_charge_sum_where('advance_payment','due_collection','status=1 and due_type=2 AND date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['operation_expense']=$this->admin_model->get_charge_sum_where('paid_cost','service_payment_details','date(created_at) between "'.$start_date.'" and "'.$end_date.'"');
		...
		// Indoor Diagnostic Service
	 	...
	 	$data['indoor_diag_net_income']=$this->admin_model->get_charge_sum_where('paid_due','due_collection','status=1 and due_type=3 AND date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['indoor_diag_vat_income']=$this->admin_model->get_charge_sum_where('vat','due_collection','status=1 and due_type=3 AND date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['indoor_diag_vat_income_today']=$this->admin_model->get_charge_sum_where_join_no_selector('d.vat','due_collection d','d.status=1 and d.due_type=3 AND date(d.created_at) between "'.$start_date.'" and "'.$end_date.'" AND date(o.created_at) between "'.$start_date.'" and "'.$end_date.'"','outdoor_service_order_info o','d.order_id=o.reg_id');

	 	$data['indoor_diag_discount_expense']=$this->admin_model->get_charge_sum_where('discount','due_collection','status=1 and due_type=3 and date(created_at) between "'.$start_date.'" and "'.$end_date.'"');

	 	$data['indoor_diag_discount_income_today']=$this->admin_model->get_charge_sum_where_join_no_selector('d.discount','due_collection d','d.status=1 and d.due_type=3 AND date(d.created_at) between "'.$start_date.'" and "'.$end_date.'" AND date(o.created_at) between "'.$start_date.'" and "'.$end_date.'"','outdoor_service_order_info o','d.order_id=o.reg_id');
...
// **************** Pathology Module  **************
	 ...
	 public function pathology_list_dt($value='')
	 {
	 	...
	 	$output = array(  
	 		...
	 		"recordsFiltered"     =>     $this->admin_model->get_filtered_data_five_table_join('opd_patient_info','opd_patient_info.status=1 and pathologoy.status=1',$select_column,$order_column,$search_column,'pathologoy','pathologoy.patient_id=opd_patient_info.id','diagnostic_test_subgroup','pathologoy.test_id=diagnostic_test_subgroup.id','diagnostic_test_group','diagnostic_test_subgroup.mtest_id=diagnostic_test_group.test_id','opd_patient_test_order_info','opd_patient_test_order_info.id=pathologoy.order_id','pathologoy_id'
	 	),  
	 		"data"                    =>     $data  
	 	);  
	 	echo json_encode($output);
	 ...
	 public function add_report_multi($patient_info_id,$test_group_id,$specimen_id,$order_id,$state="")
	 {
	 	...
	 	$data['pathology']=$this->admin_model->select_join_pathology_where('pathologoy.padate,o.uhid,is_ipd_patient,payment_status,is_heading,pathologoy_id,pathologoy.patient_id,order_id,pdate,padate,delivery_date,pathologoy.test_id,delievery_status,patient_name,sub_test_title,delievery_status,diagnostic_test_group.test_id,diagnostic_test_subgroup.ref_val,diagnostic_test_group.speciman,o.ref_doc_name,o.quack_doc_name,test_template,
		...
		// "<pre>";print_r($data['pathology']);die();
	 	...
	 	$uhid_info=$this->admin_model->select_with_where2('*','id="'.$data['pathology'][0]['uhid'].'"','uhid');

	 	if(!empty($uhid_info)){

	 		$data['uhid_gen_id']=$uhid_info[0]['gen_id'];
	 	...
	 	$data['padate']=$data['pathology'][0]['padate'];
	 ...
	 public function add_report($groupid,$testid,$pathologoy_id)
	 {
	 	...
	 	$data['pathology']=$this->admin_model->select_join_pathology_where('pathologoy.padate,o.uhid,pathologoy_id,pathologoy.patient_id,order_id,pdate,padate,delivery_date,pathologoy.test_id,delievery_status,patient_name,sub_test_title,delievery_status,
	...
	// <<<<<<<<<<<<<<<  Manage Appoinment Module Starts  >>>>>>>>>>>>>>>>>
	 ...
	 public function add_appointment_post($value='')
	 {
		...
		// insert into appointment_payment table
	 	$data1 = array('appointment_id' => $appointment_id,
	 		'total_amount' => $this->input->post('total_amount'),
	 		'discount' => $this->input->post('discount'),
	...
	//Lab Management Start
	 ...
	 public function lab_product_list_dt($value='')
	 {
	 	...
	 	$output = array(  
	 		...
	 		"recordsFiltered"     =>     $this->admin_model->get_filtered_data_three_table_join('company c',$condition,$select_column,$order_column,$search_column,'sh_tbl_lab_product p','c.id=p.p_company_id','unit_info u','u.id=p_unit_id','p.id'
	 	),  
	 		"data"                    =>     $data  
	 	);  
	 	echo json_encode($output);
	...
	//lab supplier detials
	 ...
	 public function lab_outstanding_supplier_dt($value='')
	 {
	 	...
	 	$output = array(  
	 		...
	 		"recordsFiltered"     =>     $this->admin_model->get_filtered_data_two_table_join('sh_tbl_lab_in_product b',$condition,$select_column,$order_column,$search_column,'supplier s','b.supp_id=s.id','buy_id'
	 	),  
	 		"data"                    =>     $data  
	 	);  
	 	echo json_encode($output);
