...

{
    ...
    public function indoor_bl_current_due($d)
    {
        $query = $this->db->query("SELECT sum(current_due) as current_due FROM (
          SELECT max(id) id FROM due_collection where due_type=2 and date(created_at) = '$d' and is_due_collection=0
          GROUP BY order_id
      ) d2 JOIN due_collection d1 USING (id);");

        return $query->result_array();
    }

    public function indoor_bl_current_due_datewise($s,$e)
    {
        $query = $this->db->query("SELECT sum(current_due) as current_due FROM (
          SELECT max(id) id FROM due_collection where due_type=2 and date(created_at) between '$s' and '$e' and is_due_collection=0
          GROUP BY order_id
      ) d2 JOIN due_collection d1 USING (id);");
...
public function select_join_where_group_by_right($selector,$table_name,$join_table,$join_condition,$condition,$group_by)
{
    $this->db->select($selector);
    $this->db->from($table_name);
...
public function select_join_four_table2_left($selector,$table_name,$join_table1,$join_table2,$join_table3,$join_condition1,$join_condition2,$join_condition3,$condition)
{
        // $this->db->distinct($selector);
    $this->db->select($selector);
    $this->db->from($table_name);
...
public function make_query_doc_schedule_list($order_column)
{
    ...
    FROM doctor_schedule s LEFT JOIN doctor d 
...
public function select_join_two_table2_sum_three($selector,$table_name,$join_table1,$join_condition1,$condition,$sum_selector1,$sum_selector2,$sum_selector3,$group_selector)
{
        // $this->db->distinct($selector);
    $this->db->select($selector);
    $this->db->select_sum($sum_selector1);
...
public function select_join_five_table2_sum_six_count_group_by_right($selector,$table_name,$join_table1,$join_table2,$join_table3,$join_table4,$join_condition1,$join_condition2,$join_condition3,$join_condition4,$condition,$sum_selector1,$sum_selector2,$sum_selector3,$sum_selector4,$sum_selector5,$sum_selector6,$group_selector)
{
        // $this->db->distinct($selector);
    $this->db->select($selector);
    $this->db->select_sum($sum_selector1);
    ...
    $this->db->from($table_name);
    $this->db->join($join_table1,$join_condition1);
    $this->db->join($join_table2,$join_condition2,"RIGHT");
