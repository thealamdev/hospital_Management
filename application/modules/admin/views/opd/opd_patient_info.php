                ...
                <div class="col-md-3">
                  ...
                  <div class="form-group ml-3">
                    <div class="form-check-inline">
                      <input class="form-control col-sm-4" tabindex="3" autocomplete="off" id="age_dt" name="Day" type="text" placeholder="Day">
                      <input class="form-control col-sm-4" tabindex="4" autocomplete="off" onchange="month_to_year()" id="age_mn" name="Month" type="text" placeholder="Month">
                      <input class="form-control col-sm-4" tabindex="5" autocomplete="off" id="age_yr" name="Year" type="text" placeholder="Year">
                    </div>
                  </div>
                </div>
                ...
                <div class="col-md-3">
                  <div class="form-group">
                    ...
                    <div class="input-group ml-3">
                      <input placeholder="dd/mm/yy" value="0000-00-00" autocomplete="off" type="text" onchange="comp()" name="date_of_birth" id="date_of_birth" class="col-sm-7 date-time-picker form-control date_of_birth" data-options='{"timepicker":false,"format":"Y-m-d"}' value="" tabindex="6" />
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      $(document).on('click', '.add_this_test', function() {
        ...
        var doc_com_per = $(this).data('doc-com-per');
        var quantity = "1";
        ...
        $.ajax({
          ...
          data: {
            ...
            doc_com_per: doc_com_per,
            test_name: test_name,
            test_price: test_price,
            quk_ref_com: quk_ref_com,
            quantity: quantity,
            type: type
          },
          success: function(data) {
          ...
          function() {
            ...
            $.ajax({
              ...
              success: function(data) {
                ...
                $('#net_total').val (Math.floor(net_total.toFixed(2)));
                total_paid();
                ...
                $("#total_c_o").val (Math.floor(sum.toFixed(2)));
                $("#sub_c_o").val (Math.floor(sum_sub.toFixed(2)));
                sum = 0;
                sum_sub = 0;
      ...
      $(document).on('input', '#discount', function() {
        ...
        var discount_show = $('#discount_percent');
        ...
        if (discount.val()) {
          discount_percent = (discount.val() / total) * 100;

          discount_show.val(parseFloat(discount_percent).toFixed(2));
          discount_val = discount.val();
          $('#discount_store_per').val (Math.floor(discount_percent));
          $("#discount_store").val (Math.floor(discount_percent));
        } else {
          discount_val = 0;
          discount_show.val("");
          $("#discount_store").val("");
        ...
        var rowCount = $('#test_cart_table tr').length;
        var table = document.getElementById('test_cart_table');
        //alert(total_additional_test);

        for (var i = 1; i < rowCount - total_additional_test - 9; i++) {

          table.rows[i].cells[4].innerHTML = ((parseFloat(discount_val) / (rowCount - total_additional_test - 10)).toFixed(2));
          ...
          if ($('#discount_commission_type').val() == 1) {
            ...
            if (discount_percent < parseInt(table.rows[i].cells[8].innerHTML)) {
              table.rows[i].cells[7].innerHTML = (parseFloat(table.rows[i].cells[6].innerHTML) / 2).toFixed(2);
            } else if (discount_percent > table.rows[i].cells[8].innerHTML) {
              table.rows[i].cells[7].innerHTML = 0;
            } else {
              table.rows[i].cells[7].innerHTML = parseFloat(table.rows[i].cells[6].innerHTML).toFixed(2);
        ...
        var net_total = (parseFloat(total) + parseFloat(vat)) - parseFloat(discount_val);
    ...
    function total_paid(argument) {
      ...
      /* $('#due').val(due.toFixed(2)); */
			$('#due').val (Math.floor(due.toFixed(2)));
    }
  </script>
