    ...
    <div class="row">
      ...
      foreach ($patient_order_info as $key => $value) { 
        ...
        <div class="col-md-3" style="margin-bottom: 20px !important">
          <span style="text-align: left;margin-left: 30px;font-size: 18px;font-weight: bold;">Sample Tag</span>
          <table class="farhana-table-3">
            <tr><td><b>Invoice Id: </b><label><?=$value['test_order_id']?></label></td>
            </tr>
            <tr><td><b>Date: </b><label><?=date("d-m-Y h:i:s a", strtotime($value['created_at']))?></label></td> 
            </tr>
          </table>
          <br>
          <?php if($step_size == 24) { $step_size=0; ?>
            <br> <br> <br>
          <?php }?>
        </div>
