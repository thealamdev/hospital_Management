...
<body class="light">
                  ...
                  foreach ($hospital as $key => $hos)
                    {?>
                      <tr>
                        ...
                        <td><span class="badge badge-secondary"><?=$hos['hospital_title'];?></span></td>
                        <td>
                          <?php foreach ($test as $key => $value)
                          { if($hos['hospital_id']==$value['hospital_id']) {?>


                            <a href="javascript:void(0)" class="edit_button" id="edit_<?=$value['test_id'];?>"><span class="badge badge-secondary"><?=$value['test_title'];?> (<?=$value['speciman'];?>)</span></a></br>&nbsp;&nbsp;

                            <?php foreach ($sub_test as $key1 => $row) {?>
                              <?php if($row['mtest_id']==$value['test_id']){?><a href="javascript:void(0)" class="edit_sub_span" id="edit_sub_<?=$row['id'];?>">
                                <span class="mt-2 mb-2 badge badge-pill badge-info"><?=$row['sub_test_title'];?></span>
                              </a>
                      ...
                      <td align="center">
                        <!-- <button type="button" id=""class="btn btn-success btn-xs edit_button"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button> -->
                        <!-- <button type="button" id=""class="btn btn-danger btn-xs delete_button"><i class="fa fa-trash-o" aria-hidden="true"></i></button> -->
                      </td>
  ...
  $(document).on('click','#add_button_modal', function(event)
  {
    ...
    $.ajax({  
      ...
      method:"POST",  
      data:{test_title:test_title,hospital_id:hospital_id,speciman:res[0],specimen_id:res[1]},
      dataType:"json",  
      success:function(data)  
      { 
   ...
   $(document).on('click','#update_button_modal', function(event)
   {
     ...
     var hospital_id=$( "#hospital_name_select_edit option:selected" ).val();
       ...
       $.ajax({  
        ...
        method:"POST",
        data:{test_id:test_id,test_title_edit:test_title_edit,hospital_id:hospital_id,edit_speciman:res[0],specimen_id:res[1]},  
        dataType:"json",  
        success:function(data)  
        { 
        ...
        $.ajax({  
          ...
          { 
            $.each(data['sub_test'], function (key, value) {
              ...
              $("#hospital_name_select_sub_edit").append('<option selected value="' + value.hospital_id + '">' + value.hospital_title + '</option>');
            ...
            $.each(data['test'], function (key, value) {
              if(test_id!=value.test_id && hospital_id== value.hospital_id)
              {
            ...
            $.each(data['regent_product_list'], function (key, value) {
              ...
              {
                $("#reagent_p_id_edit").append('<option selected value="' + value.id + '">' + value.p_name + '</option>');
              }
              else
              {
                $("#reagent_p_id_edit").append('<option  value="' + value.id + '">' + value.p_name + '</option>');
              }
