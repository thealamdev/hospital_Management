...
function showoperation(str) {
    if (str == "") {
        ...
        xmlhttp.open("GET","admin/search_service_cost/"+str,true);
        xmlhttp.send();
    }
}
</script>
...
function sum() {
      ...
      var s_discount = document.getElementById('s_discount').value;
      var result = parseInt(service_cost) - parseInt(s_discount);
      if (!isNaN(result)) {
...
function sum_n() {
  ...
  <?php echo validation_errors(); ?>         
      ...
      <div class="mt-sm-3 shadow-lg p-3 mb-5 rounded">
	          <form method="post" action="admin/service_patient_entry">
          <div class="row">
	...
	<input class="form-control form-control-sm" onkeyup="showUser(this.value)"  type="text" name="patient_id" placeholder="Patient ID" required>		   
              ...
              <div class="form-group">
                 <label for="email" class="col-sm-12 control-label">Service Title</label>
                 <div class="col-sm-10">
                    <select class="custom-select select2" name="service_title" onchange="showoperation(this.value)">
                        <option value="0">Select Service Title</option>
						<?php
		 ...
		 <div class="col-md-12">
		         <table id="test_table" class="table table-bordered table-hover data-tables"
                    ...
                    <tbody>
                          ...
                          <?php $i=1;
                            foreach ($service_patient_list as $key => $value) {
					...
					echo "<button type='submit' class='btn btn-success'>Paid</button>";						
									...
									{
		echo "<a href='admin/operation_due_collection/$id/$ptid/$pat_mmid/2'><button type='submit' class='btn btn-success'>Collect Due</button></a>";				
									}
